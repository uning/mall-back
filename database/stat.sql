...
-- drop database hotel_stat;
-- create database hotel_stat;
-- use hotel_stat;
-- set names utf8;
...
CREATE TABLE IF NOT EXISTS `daily_general` (
  ...
  --  经济相关
  `money` bigint(20) DEFAULT NULL COMMENT '当天金币',
  `exp` bigint(20) DEFAULT '0' COMMENT '当天产生',
  `gem` bigint(20) DEFAULT '0' COMMENT '钱币',
...
CREATE TABLE `action_history` (
  ...
  `gift_num` int(11) default '0' COMMENT '送礼次数',
   PRIMARY KEY  (`id`),
   KEY `item_date` (`action_id`,`dateday`),
   KEY `index_item` (`action_id`),
   KEY `index_dateday` (`dateday`)
)  DEFAULT CHARSET=utf8;
